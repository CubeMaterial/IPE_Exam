...
class TxtLoader:
    """TXT/Markdown 파일에서 텍스트를 읽는 로더입니다."""

    SUPPORTED_EXTENSIONS = {".txt", ".md"}

    def load(self, file_path: str | Path) -> str:
        """텍스트 파일을 UTF-8, cp949, euc-kr 순서로 읽어 텍스트를 반환합니다."""
        path = Path(file_path).expanduser()
        if not path.exists():
            raise DocumentLoadError(f"텍스트 파일을 찾을 수 없습니다: {path}")
        if path.suffix.lower() not in self.SUPPORTED_EXTENSIONS:
            raise DocumentLoadError(f"TXT 또는 Markdown 파일만 읽을 수 있습니다: {path}")
        ...
        for encoding in ("utf-8", "cp949", "euc-kr"):
            try:
                ...
                raise DocumentLoadError(f"텍스트 파일 읽기에 실패했습니다: {path}") from exc

        error_detail = " / ".join(errors)
        raise DocumentLoadError(
            f"텍스트 파일 인코딩을 읽을 수 없습니다: {path}\n"
