...
> 문제만 추출한 마크다운입니다. 정답/해설/댓글/관련글은 제외했습니다.
...
다음은 C 코드에 대한 문제이다. 아래 코드를 확인하여 알맞은 출력값을 작성하시오.
...
다음은 파이썬에 대한 문제이다. 아래 코드를 확인하여 출력값에 알맞은 값을 작성하시오.
...
대표적인 예는 소셜 로그인이며 SSO(Single Sign-On)와 달리 동일 시스템 내 인증이 아니라 서로 다른 서비스 간의 권한 위임에 초점이 맞춰져 있다.

---

## 14번

다음 아래의 테이블을 확인하여 `R % S`의 결과를 테이블 형태로 기재하시오.
...
다음은 Java에 대한 코드이다. 알맞은 출력값을 작성하시오.
...
다음은 정보보안에서 사용하는 접근통제(Access Control) 방식에 대한 설명이다.
...
| 식별자 ID | 테스트 항목 | ( ) | ( ) | ( ) |
|---|---|---|---|---|
| DS-45S-21 | 로그인 기능 | 사용자 초기 화면 | 사용자 아이디(Test11)<br>비밀번호(test@#@!#) | 로그인 성공 |
| DS-45S-25 | 로그인 기능 | 사용자 초기 화면 | 사용자 아이디(Test11)<br>비밀번호("") | 로그인 실패 |
...
다음은 SQL에 관한 문제이다. 아래 A 테이블을 참고하여 쿼리의 결과를 작성하시오.
